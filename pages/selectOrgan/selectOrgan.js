...
Page({
  ...
  bindMultiPickerChange: function(e) {
    ...
    console.log(that.data.areacodeList[that.data.multiIndex[1]]);
    //查询该地区的体检中心信息
    wx.request({
      data: {
  ...
  bindMultiPickerColumnChange: function(e) {
    switch (e.detail.column) {
      case 0:
        list = [];
        var areacodeList = [];
        for (var i = 0; i < that.data.cityVOs.length; i++) {
          if (that.data.areaVos[i].citycode == that.data.cityVOs[e.detail.value].citycode) {
            list.push(that.data.areaVos[i].areaname)
            areacodeList.push(that.data.areaVos[i]);
          }
        }
        that.setData({
          "multiArray[1]": list,
  ...
  selectOrganDate: function(e) {
    if (typeof (that.data.medicalCenterId) == "undefined" || that.data.medicalCenterId == '') {
      this.showModal('请选择体检中心');
      return false
    }
    wx.navigateTo({
      url: '../selectOrganDate/selectOrganDate?cardcode=' + that.data.cardCode + "&medicalcode=" + that.data.medicalCenterId + "&loginMethod=" + that.data.loginMethod + "&setMeal=" + that.data.setMeal,
    })
  },
